...
//    }
    ...
    static func save(rmedis:[DailyApp],completion:@escaping(Result<Int,Error>)->Void){
        do{
            let data=try JSONEncoder().encode(rmedis)
            let outfile=try AppStore.fileURL()
            try data.write(to: outfile)
            DispatchQueue.main.async {
                completion(.success(rmedis.count))
            }
        }catch{
            DispatchQueue.main.async {
                completion(.failure(error))
    ...
    static func saveSetting(setting:Setting,completion:@escaping(Result<Int,Error>)->Void){
        do{
    ...
    static func saveCsv(rmedis:[DailyApp],completion:@escaping(Result<Int,Error>)->Void){
        do{
...
//            let string=try CSVWriter.encode(rows: rmedis, into: String.self)
            ...
            try encoder.encode(rmedis, into: AppStore.fileCsvURLCreate())
